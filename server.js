...
app.post("/api/resources", async (req, res) => {
  ...
  try {
    ...
    console.log(
      chalk.green(`✅ Resource saved to database: ${savedResource._id}`),
    );
    res.status(201).json({
      success: true,
      message:
    ...
    const errorMessage =
      error.message === "Save operation timed out"
...
app.get("/api/resources", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch resources. Please try again later." });
  }
});
...
app.get("/api/admin/resources", authenticateAdmin, async (req, res) => {
  try {
    ...
    console.error(
      chalk.red(`❌ Error fetching admin resources: ${error.message}`),
    );
    res
      .status(500)
      .json({ error: "Failed to fetch resources. Please try again later." });
